...
jobs:
  oasdiff:
    runs-on: ubuntu-latest
    steps:
      - uses: oasdiff/oasdiff-action/changelog@v0.0.36
        with:
          base: 'specs/base.yaml'
          revision: 'specs/revision.yaml'
```
...
jobs:
  oasdiff:
    runs-on: ubuntu-latest
    steps:
      - uses: oasdiff/oasdiff-action/diff@v0.0.36
        with:
          base: 'specs/base.yaml'
          revision: 'specs/revision.yaml'
```
...
`oasdiff/oasdiff-action/pr-comment@v0.0.36` posts a single auto-updating comment on every PR that touches your API spec.
