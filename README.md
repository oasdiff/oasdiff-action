...
jobs:
  breaking-changes:
    ...
    steps:
      ...
      - uses: oasdiff/oasdiff-action/breaking@v0.0.58
        with:
...
jobs:
  changelog:
    ...
    steps:
      - uses: actions/checkout@v6
      - run: git fetch --depth=1 origin ${{ github.base_ref }}
      - uses: oasdiff/oasdiff-action/changelog@v0.0.58
        with:
          base: 'origin/${{ github.base_ref }}:openapi.yaml'
          revision: 'HEAD:openapi.yaml'
          github-token: ${{ github.token }}
...
jobs:
  diff:
    ...
    steps:
      - uses: actions/checkout@v6
      - run: git fetch --depth=1 origin ${{ github.base_ref }}
      - uses: oasdiff/oasdiff-action/diff@v0.0.58
        with:
          base: 'origin/${{ github.base_ref }}:openapi.yaml'
          revision: 'HEAD:openapi.yaml'
```
...
jobs:
  validate:
    ...
    steps:
      - uses: actions/checkout@v6
      - uses: oasdiff/oasdiff-action/validate@v0.0.58
        with:
          spec: 'openapi.yaml'
```
...
jobs:
  pr-comment:
    ...
    steps:
      - uses: actions/checkout@v6
      - run: git fetch --depth=1 origin ${{ github.base_ref }}
      - uses: oasdiff/oasdiff-action/pr-comment@v0.0.58
        with:
          base: 'origin/${{ github.base_ref }}:openapi.yaml'
          revision: 'HEAD:openapi.yaml'
...
jobs:
  ...
  verify:
    ...
    steps:
      ...
      - uses: oasdiff/oasdiff-action/verify@v0.0.58
        with:
